...
/**
 * This is a patched version of our provider's svganimationview. Theirs adds the buttons
 * directly to the same jpanel as the one used for drawing, causing a bug that sometimes
 * puts buttons that take up the entire view.
 */
public class SvgAnimationView implements IView<String, IAShape, IAAnimation> {
  ...
  private String drawOval(IOval c) {
    return String.format("\n<ellipse id=\"%s\" cx=\"%f\" cy=\"%f\" rx=\"%f\" " +
                    "ry=\"%f\" fill=\"rgb%s\" visibility=\"visible\">\n", c.getName(),
